...
export const useClinicalAnalysis = (): UseClinicalAnalysisReturn => {
  ...
  const [modelProgress, setModelProgress] = useState<{ text: string; percent: number } | null>(
    null,
  );
  ...
  const [, addOptimisticLog] = useOptimistic(
    logs,
    (state: AgentLogEntry[], newLog: AgentLogEntry) => [...state, newLog],
  );
  ...
  useEffect(() => {
    const initCoreServices = async () => {
      try {
        ...
        Logger.error('System', 'Core Service Init Failed', {
  ...
  const initAIServices = useCallback(async () => {
    ...
    try {
      ...
      localLLMRef.current
        ...
        .catch((e: unknown) => {
          Logger.warn('System', 'Local LLM failed, cloud fallback active', {
            error: e instanceof Error ? e.message : String(e),
          });
          setModelProgress(null);
        });
  ...
  const executeAnalysis = useCallback(async () => {
    ...
    try {
      ...
      addLog('Router-Agent', `Routed intent: ${intent} -> ${requiredSpecialist}`, 'completed', {
        intent,
      });
      ...
      if (
        !agentDBRef.current ||
        !reasoningBankRef.current ||
        !localLLMRef.current ||
        !visionSpecialistRef.current
      ) {
      ...
      addLog(
        'GOAP-Agent',
        `Plan ${trace.runId} completed in ${duration.toString()}ms`,
        'completed',
        { runId: trace.runId },
      );
