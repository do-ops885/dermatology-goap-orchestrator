      ...
      <div className="mt-3 space-y-2">
        <div>
          <div className="flex justify-between items-center mb-1">
            ...
            <span className={`text-[10px] font-bold font-mono ${isLowConfidence ? 'text-amber-600' : 'text-green-600'}`}>
...
const DifferentialDiagnosis: React.FC<{ result: AnalysisResult }> = ({ result }) => {
  const differential = (result as unknown as { differential_diagnosis?: string[] }).differential_diagnosis;
...
const AgentReasoning: React.FC<{ result: AnalysisResult }> = ({ result }) => {
  const resultWithRisk = result as unknown as { riskAssessment?: string; reasoning?: string; riskEngine?: string };
...
const SimilarCases: React.FC<{ result: AnalysisResult }> = ({ result }) => {
  const similarCases = (result as unknown as { similarCases?: Array<{ outcome?: string; context?: string; taskType?: string; score: number }> }).similarCases;
...
const WebVerification: React.FC<{ verification: AnalysisResult['webVerification'] }> = ({ verification }) => {
...
const RecommendationSection: React.FC<{ recommendations: string[] }> = ({ recommendations }) => (
  <div className="flex-1 p-4 bg-stone-50 rounded-2xl border border-stone-100">
    ...
    <p className="text-sm text-stone-700 leading-relaxed font-serif italic border-l-2 border-terracotta-300 pl-4">
      "{recommendations.length > 0 ? recommendations[0] : 'Consult a healthcare professional for follow-up.'}"
    </p>
  </div>
);

// Helper: Security footer
const SecurityFooter: React.FC<{ securityContext: AnalysisResult['securityContext'] }> = ({ securityContext }) => {
...
export const DiagnosticSummary: React.FC<DiagnosticSummaryProps> = ({ result }) => {
  ...
  const handleFeedback = async (feedback: {
    ...
    const feedbackData: ClinicianFeedbackType = {
      ...
      isCorrection: Boolean(feedback.correctedDiagnosis) && feedback.correctedDiagnosis !== feedback.diagnosis
    };
        ...
        <div className="space-y-6 flex-1 flex flex-col">
          ...
          {showFeedback && result.lesions.length > 0 && (
            <ClinicianFeedback
              ...
              onSubmit={(feedback) => { void handleFeedback(feedback); }}
