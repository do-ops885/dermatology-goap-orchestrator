  ...
  return (
    <div className={`glass-panel p-6 rounded-2xl flex-1 transition-all duration-700 flex flex-col ${result ? 'opacity-100 scale-100' : 'opacity-40 scale-95 pointer-events-none'}`}>
        <div className="flex justify-between items-start mb-6">
            ...
            {result?.securityContext?.encrypted === true ? (
        ...
        {result ? (
            <div className="space-y-6 flex-1 flex flex-col">
                <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="mt-3 space-y-2">
                          ...
                          <div>
                              <div className="flex justify-between items-center mb-1">
                                ...
                                <span className={`text-[10px] font-bold font-mono ${(result.lesions[0]?.confidence ?? 0) < 0.65 ? 'text-amber-600' : 'text-green-600'}`}>
                                  {((result.lesions[0]?.confidence ?? 0) * 100).toFixed(1)}%
                                </span>
                              </div>
                              <div className="h-1.5 bg-stone-200 rounded-full overflow-hidden">
                                  <div
                                      className={`h-full transition-all duration-1000 ${(result.lesions[0]?.confidence ?? 0) < 0.65 ? 'bg-amber-500' : 'bg-green-500'}`}
                                      style={{ width: `${((result.lesions[0]?.confidence ?? 0) * 100).toString()}%` }}
                                  />
                              </div>
                          </div>
                        </div>
                    ...
                    <div className="p-4 bg-stone-50 rounded-2xl border border-stone-100">
                        ...
                        <div className="text-xl font-bold font-grotesk text-stone-900">
                          {(result as unknown as { fairness?: number }).fairness !== undefined ? ((result as unknown as { fairness: number }).fairness * 100).toFixed(0) : '92'}%
                        </div>
                ...
                <div className="p-4 bg-stone-50 rounded-2xl border border-stone-100 relative overflow-hidden">
                     ...
                     {(result as unknown as { riskEngine?: string }).riskEngine !== undefined && (result as unknown as { riskEngine?: string }).riskEngine !== '' ? (
                ...
                <div className="flex-1 p-4 bg-stone-50 rounded-2xl border border-stone-100">
                    ...
                    <p className="text-sm text-stone-700 leading-relaxed font-serif italic border-l-2 border-terracotta-300 pl-4">
                        "{result.recommendations.length > 0 ? result.recommendations[0] : 'Consult a healthcare professional for follow-up.'}"
                    </p>
                </div>
