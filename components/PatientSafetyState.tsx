        ...
        {metrics.length > 0 && (
            <div className="grid grid-cols-2 gap-3 mb-4 p-3 bg-stone-50/50 rounded-xl border border-stone-100">
                {metrics.map(([key, value]) => {
                    ...
                    return (
                        <div key={key} className={`flex flex-col gap-1.5 p-2 rounded-lg border text-[10px] font-mono ${bg}`}>
                             <div className="flex justify-between items-center">
                                 <span className="truncate opacity-80 uppercase tracking-tighter">{key.replace(/_/g, ' ')}</span>
            ...
            {flags.map(([key, value]) => {
                ...
                return (
                    <div key={key} className={`flex items-center gap-2 px-2 py-1.5 rounded-lg border text-[10px] font-mono transition-all
                        ...
                        {isActive ? <CheckCircle2 className="w-3 h-3" /> : <div className="w-3 h-3 rounded-full border border-stone-200" />}
