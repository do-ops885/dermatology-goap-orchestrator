...
const FairnessReport: React.FC<FairnessReportProps> = ({ onClose }) => {
  ...
  >(() => agentDB.getFairnessMetrics());
  const [auditLog, setAuditLog] = useState<
    { id: string; timestamp: number; type: string; severity?: string; [key: string]: unknown }[]
  >([]);
  ...
  const handleReset = async () => {
    if (
      confirm(
        'Are you sure you want to wipe the AgentDB memory? This will reset all learned patterns.',
      )
    ) {
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-bold font-grotesk text-stone-900 flex items-center gap-3">
                ...
                <span className="text-[10px] font-mono bg-stone-100 text-stone-500 px-2 py-0.5 rounded-full border border-stone-200">
                  v3.1.2-ALPHA
                </span>
              </h2>
              <p className="text-sm text-stone-500 flex items-center gap-2">
                <Database className="w-3 h-3" /> Immutable AgentDB Ledger • Fitzpatrick Calibration
                • Equalized Odds Disentanglement
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-4 space-y-6">
              <div className="bg-slate-50 border border-slate-100 p-6 rounded-3xl shadow-sm">
                <div className="flex items-center justify-between mb-6">
                  <h4 className="text-sm font-bold text-slate-700 font-grotesk uppercase tracking-wider">
                    Executive Summary
                  </h4>
            ...
            <div className="lg:col-span-8 bg-white border border-stone-200 p-6 rounded-3xl shadow-sm flex flex-col">
              <div className="flex justify-between items-center mb-6">
                <div>
                  <h4 className="text-sm font-bold text-stone-700 font-grotesk uppercase tracking-wider">
                    Historical Bias Convergence
                  </h4>
                  <p className="text-[10px] text-stone-400 mt-1">Audit cycles (Last 7 Days)</p>
                </div>
                <div className="flex gap-4 text-[10px] font-bold">
                  <span className="flex items-center gap-1.5">
                    <div className="w-2 h-2 rounded-full bg-terracotta-400" /> PARITY
                  </span>
                  <span className="flex items-center gap-1.5">
                    <div className="w-2 h-2 rounded-full bg-slate-500" /> GAP
                  </span>
                </div>
              </div>
              ...
              <div className="flex justify-between items-center mb-6">
                <div>
                  ...
                  <p className="text-[10px] text-stone-400 mt-1">
                    Estimated vs Actual outcome probability per phenotype
                  </p>
                </div>
            ...
            <div className="lg:col-span-5 bg-white border border-stone-200 p-6 rounded-3xl shadow-sm overflow-hidden flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto space-y-4 pr-2">
                {demographicData.map((data) => (
                  <div
                    key={data.type}
                    className="flex items-center justify-between p-3 bg-stone-50 rounded-2xl border border-stone-100 hover:border-stone-300 transition-all group"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="text-[10px] font-bold text-stone-800 font-grotesk uppercase tracking-tight">
                          Phenotype {data.type}
                        </div>
                    ...
                    <div className="text-right">
                      <div className="text-[11px] font-bold font-mono text-slate-700">
                        TPR: {(data.tpr * 100).toFixed(1)}%
                      </div>
                      <div
                        className={`text-[9px] font-bold font-mono ${parseFloat(data.gap) > 0.05 ? 'text-terracotta-500' : 'text-green-600'}`}
                      >
                        Gap: {data.gap}
                      </div>
                    </div>
            ...
            <div className="px-8 py-5 border-b border-stone-100 bg-stone-50/50 flex items-center justify-between">
              <div>
                ...
                <p className="text-[10px] text-stone-400 mt-0.5">
                  Verification Chain ID: 0x82f..91a
                </p>
              </div>
                ...
                <tbody className="divide-y divide-stone-100">
                  {auditLog.map((item) => (
                    <tr key={item.id} className="hover:bg-stone-50/50 transition-colors group">
                      ...
                      <td className="px-8 py-5">
                        <span
                          className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold border uppercase tracking-tighter
                                      ${
                                        item.type === 'learning'
