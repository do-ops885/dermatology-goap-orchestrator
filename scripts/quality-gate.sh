...
# 2. Syntax Check (runs before ESLint)
if [ "$SKIP_LINT" = "false" ]; then
  echo -e "${BLUE}─────────────────────────────────────────────────────────────${NC}"
  echo -e "${BLUE}🔎 Syntax Check${NC}"
  echo -e "${BLUE}─────────────────────────────────────────────────────────────${NC}"
  run_check "TypeScript syntax" "npm run syntax:check -- --all" "true"
  echo ""
else
  echo -e "${YELLOW}⊘ Syntax check skipped (use --skip-lint)${NC}"
  echo ""
fi
...
echo -e "${BLUE}─────────────────────────────────────────────────────────────${NC}"
run_check "Sensitive data detection" "sh ./scripts/pre-commit-secrets.sh" "true"
echo ""
